...
class LobbyService(
    ...
    fun getLobbySettings(lobbyCode: String): Settings? = lobbyRepository.findLobbyByLobbyCode(lobbyCode)?.settings

    fun getPlayerNames(lobbyCode: String): MutableList<String>? = lobbyRepository.findLobbyByLobbyCode(lobbyCode)?.info?.playersName

    fun joinLobby(lobbyCode: String, username: String) : Boolean{
        val lobby = lobbyRepository.findLobbyByLobbyCode(lobbyCode) ?: return false
    ...
    fun startLobby(lobbyCode: String){
        return lobbyRepository.findLobbyByLobbyCode(lobbyCode)?.start() ?: TODO()
    }
